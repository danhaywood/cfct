...
The automation download dynamically selects the newest successful command from the command log, resolves touched eligible business tables using the same command-driven table-selection logic as the Vaadin UI, compares those tables, and returns the refreshed JSON in the same response.
If the newest successful command does not resolve any eligible touched business tables, the endpoint still succeeds and returns an empty comparison JSON document with `hasDifferences: false`, `differingTables: []`, and `comparedTables: []`.
...
The JSON contains `differingTables` for detailed difference payloads and `comparedTables` for the complete list of compared table identities.
