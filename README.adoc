...
The webapp can expose a Basic-authenticated automation API for non-interactive refresh and JSON download workflows.
...
A refresh dynamically selects the newest successful command from the command log, resolves touched eligible business tables using the same command-driven table-selection logic as the Vaadin UI, and compares those tables.

Trigger a synchronous refresh with Basic Auth:

[source,bash]
----
curl -u robot:secret -X POST http://localhost:8080/api/automation/refresh
----

Download the latest successful automation JSON result:
...
The download endpoint returns `404 Not Found` until at least one automation refresh has completed successfully since application startup.
A failed refresh reports an error response and leaves the previous successful JSON result available for download.
...
Verify unauthenticated refresh is rejected:

[source,bash]
----
curl -i -X POST http://localhost:8080/api/automation/refresh
----
...
[source,bash]
----
curl -i -u robot:wrong -X POST http://localhost:8080/api/automation/refresh
----

The expected response is `401 Unauthorized`.
Verify download before refresh returns no result:
...
The expected response is `404 Not Found` with a JSON body whose `status` is `not_found`.
Trigger the automation refresh:

[source,bash]
----
curl -i -u robot:secret -X POST http://localhost:8080/api/automation/refresh
----

The expected response is `200 OK` with JSON containing `status: completed`, `completedAt`, and a `tableCount` matching the eligible business tables touched by the newest successful fixture command.
Download the latest JSON comparison:
...
Run a second refresh and download to confirm the latest result remains available and the filename timestamp updates.
...
If a previous refresh succeeded, a subsequent download should still return the last successful JSON result.
If no refresh ever succeeded, the download should return `404 Not Found`.
