...
|cfct.webapp.comparison.left-database
|left_db
|Default source database name used for the left-hand comparison side.

|cfct.webapp.comparison.right-database
|right_db
|Default target database name used for the right-hand comparison side.

|cfct.webapp.comparison.env-file
|.env
|Path to dotenv-style values used by comparison execution defaults.

|cfct.webapp.comparison.output.format
|text
|Default comparison output format.

|cfct.webapp.comparison.output.file
|empty
|Optional output file path.
If empty, output is rendered without writing a file.

|cfct.webapp.comparison.validation.enabled
|true
|Enables login-time connectivity and required-object validation.

|cfct.webapp.comparison.validation.fail-fast
|true
|Stops login flow immediately when validation fails.
|===

Migration note:
`cfct.webapp.selection-plan.explicit.tables` has been removed.
Manual and command-driven selection in the webapp UI are now the supported table-selection paths.
...
The webapp reads login defaults and shared execution defaults from `cfct-webapp/src/main/resources/application.yml` using `cfct.webapp.comparison.*` keys.
...
* `cfct.webapp.comparison.left-database` ↔ `-l` / `--left-database`
* `cfct.webapp.comparison.right-database` ↔ `-r` / `--right-database`
* `env-file` ↔ `-e` / `--env-file`
...
The login form is pre-populated from `spring.datasource.*` plus `cfct.webapp.comparison.left-database` and `cfct.webapp.comparison.right-database` configuration values, and every field remains editable.
