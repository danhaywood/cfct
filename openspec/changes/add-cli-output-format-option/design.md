...
Streaming Excel bytes to stdout is also a poor default because accidental terminal output is unreadable and can corrupt terminal state, so Excel must require a named output file.
...
- Add a CLI output file option, named `-o`, for writing successful output bytes to a file.
- Require `-o` when `--output-format excel` is selected.
- Keep `text` as the default when no output format is specified.
- Keep stdout as the default destination for `text` and `json` when `-o` is omitted.
...
- Do not add append mode, directory creation policy, or overwrite prompts in this change.
...
- Add `--output-format` as the primary option name.
  A long option is explicit and avoids consuming short flags that may be useful for future behavior.
...
- Add `-o` as the output destination option.
  This is a conventional short flag for output files and keeps command lines concise.
  Alternative considered: use only `--output-file`, but the user specifically requested `-o` and the short form is widely understood.
...
- Change the successful CLI execution result from `String` to a small value object that carries bytes and metadata such as media type and file extension.
  This allows the command runner to write binary Excel output safely through an `OutputStream` or to a file.
...
- Route destination handling through `CliCommandRunner` rather than through the SQL Server executor.
  The executor should compare and render output, while the runner owns process-level concerns such as stdout, stderr, exit codes, and file writing.
  Alternative considered: let the executor write files, but that would mix rendering with command process I/O.

- Allow `-o` for `text` and `json` as an optional convenience, while requiring it for `excel`.
  This preserves stdout workflows for text and JSON and avoids unsafe Excel streaming.
  Alternative considered: require `-o` for all formats, but that would be a breaking change for existing text output.

## Risks / Trade-offs

- Existing users may expect to redirect Excel stdout after selecting Excel → Require `-o` and document the safer workflow.
- File writing can fail due to missing directories or permissions → Surface the failure through the existing execution error path with a clear message.
