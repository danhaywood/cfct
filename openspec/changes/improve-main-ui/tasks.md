...
- [x] 2.3 Add a fixed footer/status bar that displays configured server, left database, right database, and SQL connectivity status from typed webapp configuration and validation state.
...
- [x] 3.1 Replace the table-selection checkbox list with a Vaadin Grid in the AppLayout navigation drawer backed by the existing discovered table catalog.
...
- [x] 3.5 Preserve selected-table state and Compare button enablement when users select, deselect, sort, or filter rows, without displaying a selected-table count.

## 4. Placeholder Compare Action

- [x] 4.1 Add a right-aligned `Compare` button in the main comparison area.
...
- [x] 5.1 Add or update unit tests for selection state and Compare enablement behavior after Grid-backed selection changes.
