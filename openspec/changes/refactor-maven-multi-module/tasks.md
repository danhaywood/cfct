...
- [x] 1.2 Add module declarations for `sqlcomparer-api`, `sqlcomparer-impl`, `sqlcomparer-cli`, and `sqlcomparer-integration-tests` in dependency order.
...
- [x] 2.1 Create `sqlcomparer-api/src/main/java` and move public comparison contract types into it.
...
- [x] 3.1 Create `sqlcomparer-impl/src/main/java` and move comparison services, SQL Server readers, JSON request loading, configured comparison execution, and report renderers into it.
- [x] 3.2 Configure `sqlcomparer-impl` to depend on `sqlcomparer-api` and required implementation libraries such as Spring, Jackson, POI, and SQL Server JDBC as appropriate.
...
- [x] 4.1 Create `sqlcomparer-cli/src/main/java` and move `SqlComparerApplication` into it.
- [x] 4.2 Configure `sqlcomparer-cli` to depend on `sqlcomparer-impl`.
...
- [x] 4.5 Verify the CLI module packages with `SqlComparerApplication` as its main class.

## 5. Integration-Test Module

- [x] 5.1 Create `sqlcomparer-integration-tests/src/test/java` and move `*IT` classes, `SqlServerTestHarness`, and `DatabaseSide` into it.
- [x] 5.2 Move SQL fixtures, comparison request fixtures, logging configuration, approval files, and generated workbook test resources to `sqlcomparer-integration-tests/src/test/resources` as needed.
- [x] 5.3 Configure `sqlcomparer-integration-tests` to depend on `sqlcomparer-impl` and required test libraries.
