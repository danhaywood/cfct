...
- [x] 1.1 Create a CLI argument parser component in `cfct-cli` that reads `-S`, `-U`, `-P`, `-l`, `-r`, and `-t`.
...
- [x] 2.1 Add a CLI runner entry path from `CfctApplication.main` that invokes comparison execution from parsed arguments.
...
- [x] 4.1 Run `cfct-cli` module tests and fix regressions.
