...
#### Scenario: Each table uses its own BK index
- **WHEN** a multi-table comparison includes multiple tables
- **THEN** each table comparison discovers and uses that table's own `_BK` unique index

#### Scenario: Each table uses default ignored columns
- **WHEN** a requested table contains default ignored columns such as `id` and `version`
- **THEN** that table comparison excludes those columns using the same rules as single-table comparison
