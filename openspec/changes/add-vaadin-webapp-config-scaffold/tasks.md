...
- [x] 1.1 Add `sqlcomparer-webapp` to the root Maven reactor modules list.
- [x] 1.2 Create `sqlcomparer-webapp/pom.xml` with Spring Boot web dependencies and Vaadin Flow dependency pinned to stable 25.x.
- [x] 1.3 Add a minimal Spring Boot application entry point class in `sqlcomparer-webapp`.
...
- [x] 2.1 Add `application.yml` in `sqlcomparer-webapp` with baseline keys for server, username, password, left database, right database, table list, tables-file, env-file, output-format, and output-file.
...
- [x] 3.1 Ensure `sqlcomparer-webapp` depends on implementation or API layers without depending on CLI or integration-test modules.
