...
The `Compare` button SHALL be disabled while a compare run is in progress.
The `Compare` button SHALL return to enabled after compare completion when one or more eligible tables remain selected.
...
#### Scenario: Compare button disables while run is active
- **WHEN** an enabled compare run is started
- **THEN** the `Compare` button is disabled until that run reaches a terminal state

#### Scenario: Compare button re-enables after completion with eligible selection
- **WHEN** a compare run completes
- **AND** one or more eligible tables remain selected
