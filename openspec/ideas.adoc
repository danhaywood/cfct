...
* /opsx-propose: rationalize the config properties.  Currently they are all under 'cfct.webapp', but some of them only really relate to cli, and I'd like to rename others:

cfct.webapp.comparison.connection.left-database: remove the 'comparison' part, ie cfct.webapp.connection.left-database
cfct.webapp.comparison.connection.right-database: remove the 'comparison' part
cfct.webapp.comparison.env-file: remove, relates only to CLI
cfct.webapp.comparison.output.format: remove, relates only to CLI
cfct.webapp.comparison.output.file: remove, relates only to CLI
cfct.webapp.comparison.validation.enabled: remove the 'comparison' part
cfct.webapp.comparison.validation.fail-fast: remove the 'comparison' part

analyze the cfct.webapp.selection-plan.explicit.tables config prop: what uses it?  My analysis is that this is only used by beans (SelectionPlan, WebappComparisonPreparationService) that have no function - they are tested, but not wired into prod anywhere.  Is this is the case, could they be removed?
